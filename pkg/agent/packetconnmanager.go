...
type packetConn struct {
	id         int64
	conn       net.Conn
	targetAddr string // target address for this connection
	ctx        context.Context
...
func newPacketConnectionManager(ctx context.Context) packetConnManager {
	return newPacketConnectionManagerWithConfig(ctx, DefaultPacketConnManagerConfig())
}
...
func (p *packetConnManagerImpl) createConnection(packet *v1.Packet) error {
	...
	targetAddr := packet.TargetAddress
	klog.V(4).InfoS("Target address resolved", "conn_id", connID, "target", targetAddr)
	...
	klog.V(4).InfoS("Successfully connected to target", "conn_id", connID, "target", targetAddr)
