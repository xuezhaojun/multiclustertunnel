...
type Config struct {
	...
	// Router implementation for handling HTTP requests
	Router Router
...
func New(config *Config) (*Server, error) {
	...
	// Create HTTP server if router is provided
	if config.Router != nil {
		handler := &httpHandler{tunnelManager: tunnelManager, router: config.Router}
...
// CreateServerWithConfig creates a hub server with custom configuration
func CreateServerWithConfig(config *Config) (*Server, error) {
	return New(config)
}
...
func (h *httpHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// Parse cluster name and target address from request using the router
	clusterName, targetAddress, err := h.router.Parse(r)
	if err != nil {
	...
	klog.V(4).InfoS("Routing request to cluster", "cluster", clusterName, "target_address", targetAddress, "path", r.URL.Path)
...
func (h *httpHandler) sendInitialHTTPRequest(pc packetSender, r *http.Request, targetAddress string) error {
